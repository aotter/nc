...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    const props = __props;
    useCssVars((_ctx) => ({
      "9b99b4de": unref(aspectRatioCSSVariable)
    }));
    ...
    const { t } = useI18n();
    const { view, ripple, styles, property } = useEcho(props.ncId);
    ripple.listen(INTERSECTIONS, noop, {
      events: property.value.view.event.intersections
    });
    ripple.listen(READY, (src) => {
      ready.status = true;
      ready.src = src;
    });
