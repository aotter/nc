...
class PondInstance {
  constructor(id) {
    this._id = id;
    const viewDebugFlag = viewEcho[this._id].debug;
    const developmentDebugFlag = false;
    const urlDebugFlag = !!new URLSearchParams(window.location.search).get(NC_DEBUG_PARAM);
    const debug = [viewDebugFlag, developmentDebugFlag, urlDebugFlag].some((flag) => flag);
    this._debug = debug;
  }
  get isDebug() {
    return this._debug;
  }
  _pretty(type, scope, content) {
    if (!this.isDebug)
      return;
    const color = PondColor[type];
    const emoji = PondScopeEmoji[scope];
    console.group(`%c ${emoji} ${NC_PREFIX}${scope} %c ${type.toUpperCase()} %c- ${this._id} %c`, `background: ${color}; border: 1px solid ${color}; padding: 2px; border-radius: 4px; color: #fff;`, `color: ${color};`, `color: ${color};`, `background: transparent;`);
    console.log("\u21B3", content);
    console.groupEnd();
  }
  error(scope, content) {
    this._pretty(ERROR, scope, content);
    return this;
  }
  info(scope, content) {
    this._pretty(INFO, scope, content);
    return this;
  }
  log(scope, content) {
    this._pretty(LOG, scope, content);
    return this;
  }
  warn(scope, content) {
    this._pretty(WARN, scope, content);
    return this;
  }
}
const initPond = (id) => {
...
const initComponents = () => {
  ...
  if (hasDefined)
    return;
  const viewComponents = { "/src/components/view/View.ce.vue": () => import("./8711b12f.js"), "/src/components/view/layout/Default.ce.vue": () => import("./7068b554.js"), "/src/components/view/layout/Template.ce.vue": () => import("./ee23aaa7.js"), "/src/components/view/type/Banner.ce.vue": () => import("./04d356ce.js"), "/src/components/view/type/Chest.ce.vue": () => import("./2426151a.js"), "/src/components/view/type/Parallax.ce.vue": () => import("./07b239fb.js"), "/src/components/view/type/Scratch.ce.vue": () => import("./b64a3a91.js"), "/src/components/view/type/Vast.ce.vue": () => import("./6f2881ee.js") };
  const sharedComponents = { "/src/components/shared/Picture.ce.vue": () => import("./c6ddb0ad.js") };
